...
class EagleDraftWorker(EagleDraftWorkerBase):
    ...
    def _draft_extend_plan_for_decode(self, batch: ScheduleBatch):
        """Build and stage the draft-extend graph inputs before the verify
        launch, so their shared-buffer reads (metadata pool gathers) precede
...
class EAGLEWorkerV2(BaseSpecWorker):
    ...
    def forward_batch_generation(
        ...
        if batch.forward_mode.is_extend() or batch.is_extend_in_batch:
            ...
            # Plan draft_extend now so its shared-buffer reads land before
            # the verify-time WAR read-done publish.
