...
class Engine(EngineScoreMixin, EngineBase):
    ...
    def _launch_subprocesses(
        ...
        # Resolution is complete (late resolution refuses a published config),
        # so publish before the launch steps read any configuration. This
        # process goes on to host the tokenizer manager or the multi-tokenizer
        # router; the tokenizer's own publish re-projects the same object.
        publish(server_args, role="tokenizer")

        # Launch daemons (daemon mode only). Handles are threaded back to the
        # owning Engine instance rather than a class attribute, so daemon
        # ownership follows the instance lifecycle.
