...
SGLang-Diffusion supports quantized transformer checkpoints and selected native
text encoders. Transformer and text-encoder precision are independent controls.
...
- `--text-encoder-quantization fp8`: online FP8 for a compatible native text encoder
- `--text-encoder-path`: replace a text encoder with a pre-quantized checkpoint whose `quantization_config` is auto-detected
- `--text-encoder-quantization-ignored-layers`: keep matching text-encoder linears unquantized during online FP8
...
MiniMax-H3 can quantize the language linear layers in its native Qwen3-VL text
encoder independently of the DiT. Embeddings, normalization layers, and the
Qwen vision tower remain in the configured BF16 precision.

To quantize the BF16 text-encoder weights during server startup:
...
To load the official serialized FP8 Qwen3-VL checkpoint instead:

```bash Command
sglang serve \
  --model-path MiniMaxAI/MiniMax-H3 \
  --model-variant fl2va \
  --text-encoder-path Qwen/Qwen3-VL-32B-Instruct-FP8 \
  --num-gpus 4 \
  --port 30010
```

Do not use `--text-encoder-quantization-ignored-layers` to reinterpret a
serialized checkpoint: its FP8/BF16 layout is fixed by the checkpoint. Text
encoder FP8 is approximate, is not enabled by default, and is rejected by
MiniMax-H3's strict `quality="high"` deployment contract.

Online FP8 first loads the source-precision checkpoint before quantizing it. It
reduces steady-state text-encoder residency, but not the source-weight startup
peak. Use the serialized FP8 checkpoint when loading peak is the constraint.
