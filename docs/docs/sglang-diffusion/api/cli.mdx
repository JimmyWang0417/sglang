...
Native text encoders use separate controls so changing DiT precision never
silently changes prompt embeddings:

- `--text-encoder-quantization fp8` applies online FP8 to a compatible native text encoder
- `--text-encoder-quantization-ignored-layers` keeps matching native text-encoder linears in their source precision during online quantization
- `--text-encoder-path {MODEL}` selects a pre-quantized text-encoder checkpoint; its `quantization_config` is auto-detected
